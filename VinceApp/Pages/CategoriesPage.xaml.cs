...
namespace VinceApp.Pages
{
    public partial class CategoriesPage : Page
    {
        ...
        private void LoadCategories()
        {
            ...
            {
                MessageBox.Show($"خطأ في التحميل: {ex.Message}","Error",MessageBoxButton.OK,MessageBoxImage.Error);
            }
        }
                ...
                if (MessageBox.Show("هل أنت متأكد من الحذف؟", "تأكيد", MessageBoxButton.YesNo, MessageBoxImage.Warning) == MessageBoxResult.Yes)
                {
                    try
                    {
                        using (var context = new VinceSweetsDbContext())
                        {
                            ...
                            if (hasProducts)
                            {
                                MessageBox.Show("لا يمكن حذف هذا التصنيف لأنه يحتوي على منتجات.\nيرجى حذف المنتجات أو نقلها أولاً.", "منع الحذف", MessageBoxButton.OK, MessageBoxImage.Error);
                                return;
                            }
                    ...
                    {
                        MessageBox.Show($"لا يمكن الحذف: {ex.Message}");
                    }
                }
            }
        }
